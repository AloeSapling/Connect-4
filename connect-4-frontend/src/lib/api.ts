...
export async function CreateGame(lobbyCode: string) {
	await api.post('/game/create',
		proto.routes.CreateGameRequest.encode({ code: lobbyCode }).finish(),
	);
}

/** Joins the lobby associated with the provided code */
export async function JoinLobby(lobbyCode: string) {
	await api.post('/lobby/join/',
		proto.routes.JoinLobbyRequest.encode({ code: lobbyCode }).finish()
	);
}

export async function AssignPlayerID(lobbyCode: string, uid: number, playerID: proto.shared.PlayerIDs) { }
