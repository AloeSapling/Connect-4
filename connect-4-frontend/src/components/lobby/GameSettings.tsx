...
export default function GameSettings({ lobbyCode, settings }: { lobbyCode: string; settings: models.ILobbySettings }) {
    ...
    return (
        <div className="flex flex-col flex-1 gap-3 bg-yellow-900 p-3 rounded-lg">
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-2 content-center items-center">
                ...
                <div className="flex flex-col gap-2 w-full">
                    {SPECIAL_TOKEN_TYPES.map((tokenType) => (
                        <div key={tokenType} className="flex items-center gap-2">
                            <Switch
                                checked={(form.watch('allowed_tokens') ?? []).includes(tokenType)}
                                onCheckedChange={(checked) => {
                                    const current = form.getValues('allowed_tokens') ?? [];
                                    if (checked) {
                                        form.setValue('allowed_tokens', [...current, tokenType]);
                                    } else {
                                        form.setValue(
                                            'allowed_tokens',
